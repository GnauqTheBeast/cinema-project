...
function chunkByParagraph(text: string, config: ChunkConfig): TextChunk[] {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (currentChunk.length > 0 && currentChunk.length + paragraph.length + 2 > config.maxSize) {
      // Save current chunk
      const chunkContent = currentChunk.trim();
      if (chunkContent.length >= config.minSize) {
        chunks.push({
...
function getOverlapText(sentences: string[], currentIndex: number, maxOverlapChars: number): string {
  if (currentIndex === 0) {
    return '';
  }
