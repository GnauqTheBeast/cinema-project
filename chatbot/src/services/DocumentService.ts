...
export class DocumentService {
  ...
  async processDocument(filePath: string, title: string): Promise<Document> {
    ...
    this.processChunks(doc, content).catch((error) => {
      ...
      this.documentDatastore.updateDocumentStatus(doc.id, DocumentStatus.FAILED).catch((err) => {
        logger.error('Failed to update document status to failed', { docId: doc.id, error: err });
      });
    });
  ...
  private async processChunks(doc: Document, content: string): Promise<void> {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        logger.error('Failed to generate embedding for chunk', { docId: doc.id, chunkIndex: i, error });
        throw error;
      }
    }
    ...
    logger.info('Document processing completed', { docId: doc.id, chunksCount: docChunks.length });
  }
