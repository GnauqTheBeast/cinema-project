...
export default function StaffManagementPage() {
  ...
  const submitCreate = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/auth/staff`, form, {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-700 mb-6">
              Bạn có chắc chắn muốn tạo tài khoản nhân viên với email {form.email}?
            </p>
            <div className="flex justify-end space-x-3">
              <button onClick={() => setConfirmOpen(false)} className="px-4 py-2 rounded border">
                Huỷ
              </button>
              <button onClick={submitCreate} className="px-4 py-2 rounded bg-red-600 text-white">
                Xác nhận
              </button>
            </div>
          </div>
